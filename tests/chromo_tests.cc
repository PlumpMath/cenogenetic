...
int main() {
	...
	g.funcs.push_back(std::unique_ptr<tfunc>(new tfunc([](double a, double b, double c) { return a * b * c;})));
	g.funcs.push_back(std::unique_ptr<tfunc>(new tfunc([](double a, double b, double c) { return a + b + c;})));
	g.funcs.push_back(std::unique_ptr<tfunc>(new tfunc([](double a, double b, double c) { return a - b - c;})));
g.funcs.push_back(std::unique_ptr<tfunc>(new tfunc([](double a, double b, double c) { return a > b ? a > c ? a : c : b > c ? b : c;})));
g.funcs.push_back(std::unique_ptr<tfunc>(new tfunc([](double a, double b, double c) { return a < b ? a < c ? a : c : b < c ? b : c;})));
g.funcs.push_back(std::unique_ptr<tfunc>(new tfunc([](double a, double b, double c) { return a > b ? a < c ? a : b < c ? b : c : a > c ? c : a;})));
